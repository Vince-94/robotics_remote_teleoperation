...
WS_URL = f"ws://{HOST}:{WS_PORT}/?token={TOKEN}"
...
async def client_send_and_expect():
    try:
        async with connect(WS_URL) as ws_a, connect(WS_URL) as ws_b:
            # send initial auth message for both clients (server accepts either URL token or this)
            await ws_a.send(json.dumps({"token": TOKEN}))
            await ws_b.send(json.dumps({"token": TOKEN}))
